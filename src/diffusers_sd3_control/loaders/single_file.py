...
def _map_component_types_to_config_dict(component_types):
    ...
    for component_name, component_value in component_types.items():
        ...
        if is_diffusers_model and component_name not in SINGLE_FILE_OPTIONAL_COMPONENTS:
            config_dict[component_name] = ["diffusers_sd3_control", component_value[0].__name__]
            ...
            if is_scheduler_enum:
                ...
                config_dict[component_name] = ["diffusers_sd3_control", "DDIMScheduler"]
    ...
    def from_single_file(cls, pretrained_model_link_or_path, **kwargs):
        ...
        if not os.path.isdir(default_pretrained_model_config_name):
            ...
            try:
                ...
                # `local_files_only=True` but a local diffusers_sd3_control format model config is not available in the cache
                ...
                if original_config is None:
                    ...
                    logger.warning(
                        ...
                        "To avoid this warning, please explicity pass the `config` argument to `from_single_file` with a path to a local diffusers_sd3_control model repo \n"
                        "e.g. `from_single_file(<my model checkpoint path>, config=<path to local diffusers_sd3_control model repo>) \n"
