...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = f"diffusers_sd3_control/{__version__}; python/{sys.version.split()[0]}; session_id/{SESSION_ID}"
    ...
    try:
        ...
        if from_training:
            ...
            if model_description is None:
                model_description = f"This is the model card of a 🧨 diffusers_sd3_control {component} that has been pushed on the Hub. This model card has been automatically generated."
...
def populate_model_card(model_card: ModelCard, tags: Union[str, List[str]] = None) -> ModelCard:
    ...
    if model_card.data.library_name is None:
        model_card.data.library_name = "diffusers_sd3_control"
...
old_diffusers_cache = os.path.join(hf_cache_home, "diffusers_sd3_control")
    ...
    for old_blob_path in old_cache_dir.glob("**/blobs/*"):
        if old_blob_path.is_file() and not old_blob_path.is_symlink():
            ...
            try:
                ...
                logger.warning(
                    "Could not create symlink between old cache and new cache. If you use an older version of diffusers_sd3_control again, files will be re-downloaded."
                )
...
if cache_version < 1:
    ...
    if old_cache_is_not_empty:
        logger.warning(
            ...
            "later by calling `diffusers_sd3_control.utils.hub_utils.move_cache()`."
        )
        try:
            move_cache()
    ...
    if os.path.isfile(pretrained_model_name_or_path):
        ...
        if (
            ...
            try:
                ...
                warnings.warn(
                    f"Loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'` is deprecated. Loading instead from `revision='main'` with `variant={revision}`. Loading model variants via `revision='{revision}'` will be removed in diffusers_sd3_control v1. Please use `variant='{revision}'` instead.",
                    FutureWarning,
                )
                return model_file
            except:  # noqa: E722
                warnings.warn(
                    f"You are loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'`. This behavior is deprecated and will be removed in diffusers_sd3_control v1. One should use `variant='{revision}'` instead. However, it appears that {pretrained_model_name_or_path} currently does not have a {_add_variant(weights_name, revision)} file in the 'main' branch of {pretrained_model_name_or_path}. \n The Diffusers team and community would be very grateful if you could open an issue: https://github.com/huggingface/diffusers/issues/new with the title '{pretrained_model_name_or_path} is missing {_add_variant(weights_name, revision)}' so that the correct variant file can be added.",
                    FutureWarning,
                )
        try:
