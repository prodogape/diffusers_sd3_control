...
class StateDictType(enum.Enum):
    ...
    KOHYA_SS = "kohya_ss"
    PEFT = "peft"
    DIFFUSERS = "diffusers_sd3_control"
...
def convert_state_dict_to_peft(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to the PEFT format The state dict can be from previous diffusers_sd3_control format (`OLD_DIFFUSERS`), or
    new diffusers_sd3_control format (`DIFFUSERS`). The method only supports the conversion from diffusers_sd3_control old/new to PEFT for now.
    ...
    if original_type is None:
        # Old diffusers_sd3_control to PEFT
...
def convert_state_dict_to_diffusers(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to new diffusers_sd3_control format. The state dict can be from previous diffusers_sd3_control format
    (`OLD_DIFFUSERS`), or PEFT format (`PEFT`) or new diffusers_sd3_control format (`DIFFUSERS`). In the last case the method will
    return the state dict as is.

    The method only supports the conversion from diffusers_sd3_control old, PEFT to diffusers_sd3_control new for now.
...
def convert_unet_state_dict_to_peft(state_dict):
    r"""
    Converts a state dict from UNet format to diffusers_sd3_control format - i.e. by removing some keys
    """
